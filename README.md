CI-Mongo - MongoDB Library for Codeigniter 3
=======
## Support
* PHP 7+
* Codeigniter 3+
...
* `row` Retrieve single document from collection
* `result` Retrieve all documents from collection

## Update Method
* `set` Sets a field to a value
* `unset` Unset a field
* `add_to_set` Adds a value to an array if doesn't exist
...
* `increment` Increments the value of a field
* `multiple` Multiple the value of a field
...
## Extra Helper
* `timestamp` Create or convert date to MongoDB based timestamp
